...
getwd()
#setwd("")
sc22<-readRDS("~/Documents/hu_scRNA/repeated/BC22_OS.rds")
data.countbc22<-as.matrix(sc22@assays$RNA@counts)
sim <- SingleCellExperiment(assays = List(counts = data.countbc22))
#sim
#sim <- sim[tmp, ]
#dim(sim)
...
#table(geneFilter)
sim <- sim[sc22@assays$RNA@var.features, ]
dim(sim)


pc22<-log1p(assays(sim)$counts)
pca <- prcomp(t(pc22), scale. = FALSE)
rd1 <- pca$x[,1:15]
plot(rd1, col = rgb(0,0,0,.5), pch=16, asp = 1)


library(destiny, quietly = TRUE)
dm <- DiffusionMap(t(pc22))
rd2 <- cbind(DC1 = dm$DC1, DC2 = dm$DC2)

plot(rd2, col = rgb(0,0,0,.5), pch=16, asp = 1)

plot(dm,1:2,col=sc22$harmony_cell_subgroup)
plot(dm,1:2,col=cors)
?plot
cors
...
plot_dm_3D(
  dm=dm, 
  dc=c(1:3),
  condition=sc22$harmony_cell_subgroup, 
  colour=cors
)


rgl.postscript("bc22.svg", fmt = "svg", drawText = TRUE )
...
sc22_lineage <- get_lineage(
  dm=dm, 
  dim=c(1:2), 
  condition=sc22$harmony_cell_subgroup,
  start="Chondrocyte",
  end=c("Osteoblast_pro_2", "Osteoblast_4"),
  shrink.method="tricube"
)

source("/Users/peizhanchen/Downloads/mouse-xx-xy-C1-e00dc2c8fe9e1bb28c34f6776979c882f0f8e81c/step5-Slingshot/functions.R")
sc22_pseudotime <- get_pseudotime(sc22_lineage, wthres=0.9)


summary(sc22_pseudotime)
dev.off()
colors <- colorRampPalette(brewer.pal(10,'Spectral'))(100)
plotcol <- colors[cut(sc22_pseudotime, breaks=10)]

reducedDims(sim) <- SimpleList(PCA = rd1, DiffMap = rd2)
plot(reducedDims(sim)$DiffMap,  pch=16, asp = 0.5,col = sc22$harmony_cell_subgroup)

plot(reducedDims(sim)$DiffMap,  pch=16, asp = 0.5,col = plotcol)

plot(dm,1:2,col=sc22$harmony_cell_subgroup)
lines(SlingshotDataSet(sc22_lineage), lwd=2, col=c('black',"red"))

####identify the genes change along with the pseudotime
pseudotime_sc22<-sc22_pseudotime
pseudotime_lin <- pseudotime_sc22[,"curve1"]
max_pseudotime <- max(pseudotime_lin, na.rm = TRUE)
pseudotime_lin1_percent <- (pseudotime_lin*100)/max_pseudotime

pseudotime_lin <- pseudotime_sc22[,"curve2"]
max_pseudotime <- max(pseudotime_lin, na.rm = TRUE)
pseudotime_lin2_percent <- (pseudotime_lin*100)/max_pseudotime

pseudotime_sc22[,"curve1"] <- pseudotime_lin1_percent
pseudotime_sc22[,"curve2"] <- pseudotime_lin2_percent

pseudotime_sc22<-as.data.frame(pseudotime_sc22)

# select the ptime values 
ptime_1 <- pseudotime_sc22$curve1

# get cells in that lineage
lineage_cells_1 <- rownames(pseudotime_sc22)[!is.na(ptime_1)]
lineage_cells_1[1:5]
# remove values for cells not in the lineage
ptime_1 <- ptime_1[!is.na(ptime_1)]

# just test variable genes to save some time
genes_to_test <- VariableFeatures(sc22)[1:3000]

# get log normalized data to test
cnts <- pc22[genes_to_test, lineage_cells_1]
...
to_plot <- as.matrix(pc22[res$id[1:100], lineage_cells_1])

# arrange cells by pseudotime
ptime_order <- colnames(to_plot)[order(ptime_1)]
to_plot<-to_plot[,ptime_order]

cell_group<-as.vector(sc22@meta.data[ptime_order,]$harmony_cell_subgroup)
cell_group
...
######cell fate 2######
# select the ptime values 
ptime_2 <- pseudotime_sc22$curve2

# get cells in that lineage
lineage_cells_2 <- rownames(pseudotime_sc22)[!is.na(ptime_2)]
...
# get log normalized data to test
cnts <- pc22[genes_to_test, lineage_cells_2]
...
# get log normalized counts 
to_plot <- as.matrix(pc22[res$id[1:100], lineage_cells_2])
...
save.image(file="BC22_trajectory.RData")
load(file="BC22_trajectory.RData")
